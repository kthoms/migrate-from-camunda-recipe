...
class TryCatchAssertionsToAssertThatThrownByTest implements RewriteTest {
    @Override
    public void defaults(RecipeSpec spec) {
        spec.recipe(new TryCatchAssertionsToAssertThatThrownBy());
    }

    @Test
    void rewriteTryCatchToAssertThatThrownBy() {
        rewriteRun(spec -> spec.cycles(4).expectedCyclesThatMakeChanges(2), java("""
            package org.operaton.rewrite.tests;
          ...
          public class Sample {
              ...
              public void testDeleteTaskCommentWithoutAuthorization() {
                  ...
                  createTask(TASK_ID);
                  Comment createdComment = createComment(TASK_ID, null, "aComment");
                  String createdCommentId = createdComment.getId();
          
                  // when
                  assertThatThrownBy(() -> taskService.deleteTaskComment(TASK_ID, createdCommentId))
                      // then
                      .withFailMessage("Exception expected: It should not be possible to delete a comment.")
